async function fazBlip() {
  ...
  var vetorRN_SAC = vetor_RN[2][1];

  for (var i = 0; i <= vetorRN_SAC.length; i++) {
    if (vetorRN_SAC[i].name == "ContactCenter") {
      var CC_RN_hd = vetorRN_SAC[i].closedTickets;
      var CC_RN_open_hd = vetorRN_SAC[i].waitingTickets;
      var CC_RN_tma_hd = vetorRN_SAC[i].averageAttendanceTime;
      if (vetorRN_SAC[i].averageWaitTime == undefined) {
        var CC_RN_wait_hd = "00:00:00";
      } else {
        var CC_RN_wait_hd = vetorRN_SAC[i].averageWaitTime;
      }
  
                      var totalSAC = CC_RN_hd;
                      var atendidasSAC = CC_RN_open_hd;
                      var CC_RN_tme_SAC = CC_RN_wait_hd;
                      var CC_RN_tma_SAC = CC_RN_tma_hd;
                      console.log(CC_RN_tma_hd)

                      document.getElementById("totalSAC").innerHTML = totalSAC;
                      document.getElementById("total_emEsperaSAC").innerHTML = atendidasSAC;
                      document.getElementById("total_tmeSAC").innerHTML = CC_RN_tme_SAC;
                      document.getElementById("total_tmaSAC").innerHTML = CC_RN_tma_SAC;
